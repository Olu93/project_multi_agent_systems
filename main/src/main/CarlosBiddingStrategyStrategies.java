...
import negotiator.boaframework.offeringstrategy.anac2011.TheNegotiator_Offering;
import negotiator.boaframework.offeringstrategy.anac2012.IAMHaggler2012_Offering;

public class CarlosBiddingStrategyStrategies extends OfferingStrategy {
	/**
	 *
	 */
	...
	private List<OfferingStrategy> candidateStrategies;
	...
	public void init(NegotiationSession negotiationSession, OpponentModel opponentModel, OMStrategy omStrategy,
		...
		OfferingStrategy hh = new HardHeaded_Offering();
		hh.init(negotiationSession, opponentModel, omStrategy, parameters);
		OfferingStrategy ntft = new IAMhaggler2010_Offering();
		ntft.init(negotiationSession, opponentModel, omStrategy, parameters);
		OfferingStrategy cc = new TheNegotiator_Offering();
		cc.init(negotiationSession, opponentModel, omStrategy, parameters);
		this.candidateStrategies = Arrays.asList(hh, ntft, cc);
	...
	private void expandNode(final Node node) {
		node.addChild(new Node().setParent(node).setBid(candidateStrategies.get(0).determineNextBid()))
				.addChild(new Node().setParent(node).setBid(candidateStrategies.get(1).determineNextBid()))
				.addChild(new Node().setParent(node).setBid(candidateStrategies.get(2).determineNextBid()));
