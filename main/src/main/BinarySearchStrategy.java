...
public class BinarySearchStrategy extends OfferingStrategy {
    ...
    public BidDetails determineNextBidFromInput(Bid opponentLastBid, NegotiationSession negotiationSession) {
        if(IS_VERBOSE) System.out.println("negotiationSession " + negotiationSession);

        double myUtilityOfOpponentLastBid = negotiationSession.getUtilitySpace().getUtility(opponentLastBid);
        if(IS_VERBOSE) System.out.println(negotiationSession.getOpponentBidHistory().getHistory());
        if(IS_VERBOSE) System.out.println(negotiationSession.getOpponentBidHistory().getLastBidDetails().getMyUndiscountedUtil());
        ...
        if (myUtilityOfOpponentLastBid > ourMaxBidThatWeGotFromOpponent) {
            if(IS_VERBOSE) System.out.println("Binary Tactic");
    ...
    private List<BidDetails> getBidsOfUtility(double lowerBound, double upperBound) {
        List<BidDetails> candidateBids = outcomeSpace.getBidsinRange(new Range(lowerBound, upperBound));
